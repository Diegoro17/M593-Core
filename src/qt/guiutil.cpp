...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter an M593 address (e.g. %1)").arg(
...
{
    // Return if URI is not valid or is not an M593 payment URI.
    if(!uri.isValid() || uri.scheme().compare(QString("m593"), Qt::CaseInsensitive) != 0)
        return false;
...
{
    // Convert m593:// to m593:
    //
    //    Cannot handle this later, because m593:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("m593://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "m593:");
    }
...
{
    QString ret = QString("m593:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "M593.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "M593 (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("M593 (%s).lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=M593\n";
        else
            optionFile << strprintf("Name=M593 (%s)\n", chain);
